...
- **`SpacesModel.lua`** — pure state, no side effects: window↔virtual-space maps, focused window per space, tab group registry (`_tabGroups`, `_windowToTabGroup`). `categorizeWindowsForTransition` splits a switch into `toActive`/`toStorage` window lists, which `init.lua`'s `_switchSpaces` moves via the strategy.
